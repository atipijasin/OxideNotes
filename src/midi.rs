...
pub fn read_file<P: AsRef<Path>>(path: P) -> Result<Vec<MidiNote>, Box<dyn std::error::Error>> {
    ...
    let ticks_per_second = match smf.header.timing {
        Timing::Metrical(ticks_per_beat) => {
            ...
            let a = (ticks_per_beat * beats_per_minute) / 60.0;
            a
        }
        Timing::Timecode(fps, subframes) => {
            let a = fps.as_f32() as f64 * subframes as f64;
            a
        }
    };
            ...
            if let TrackEventKind::Midi { channel, message } = event.kind {
                match message {
                    MidiMessage::NoteOn { key, vel } if vel.as_int() > 0 => {
                        active_notes.insert((channel.as_int(), key.as_int()),
                        (current_time_seconds, vel.as_int()),
                        );
                    }
                    MidiMessage::NoteOff {key, vel} | MidiMessage::NoteOn { key, vel } => {
                        if let Some((start_time, velocity)) = active_notes.remove(&(channel.as_int(), key.as_int())) {
                            notes.push(MidiNote {
